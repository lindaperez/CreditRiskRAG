...
\appendix
\section{Appendix}
Project repository: \href{https://github.com/lindaperez/CreditRiskRAG}{lindaperez/CreditRiskRAG}. Repository artifacts support reproduction from raw data to final outputs. Place the Kaggle accepted and rejected LendingClub files under \texttt{Data/archive/}; run accepted-loan EDA, cleaning, preprocessing, model notebooks 1--9, SHAP interpretation, corpus chunking, retrieval indexing, reason selection, RAG/no-RAG generation, judging, and result aggregation in order. Main artifact folders are \texttt{EDA/accepted\_eda\_outputs/}, \texttt{Cleaning/cleaning\_outputs/}, \texttt{Modeling/Preprocessing/preprocessing\_outputs/}, \texttt{Modeling/modeling\_outputs/}, \texttt{Interpretation\_SHAP/shap\_outputs/}, \texttt{corpus/}, \texttt{retrieval/}, and \texttt{generation/output/}.

Installation and setup: use Python 3.11. Clone the repository with \texttt{git clone https://github.com/lindaperez/CreditRiskRAG.git} and enter the project folder. For \texttt{venv}, run \texttt{python3.11 -m venv .venv}, \texttt{source .venv/bin/activate}, \texttt{python -m pip install --upgrade pip}, and \texttt{python -m pip install -r requirements.lock.txt}. For Conda, run \texttt{conda env create -f environment.yml} and \texttt{conda activate credit-risk-rag}. For notebook execution, register the kernel with \texttt{python -m ipykernel install --user --name credit-risk-rag --display-name "Python (credit-risk-rag)"}. The README also recommends running \texttt{python scripts/reproducibility\_check.py}, or the faster development check with \texttt{--skip-package-check --skip-hash-check}.
